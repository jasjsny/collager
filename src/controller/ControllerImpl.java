...
/**
 * Controller for a collage maker which takes in inputs and delegates to view and model.
 * Allows for input to be received which will correspond to different actions being taken
 * on the project, and can communicate information to be displayed to the user via the view.
 * Adhering to the MVC design pattern, this controller is meant to receive input from the
 * user and call methods to the model/view.
 */
public class ControllerImpl implements CollageController {
   ...
   * in which the model, view, and input are provided.
   * All input is received from the Readable object, which is used to determine the necessary
   * actions to be taken on the model or view.
   *
   * @param model the model which will have its methods called by this controller
   *              depending on the input received
   * @param view  the visual which will display a representation
   *              of the model to the user
   * @param input a readable which is used to take inputs from a user,
   *              and which is read with a scanner
   */
  ...
  /**
   * Controller method to start the controller. Beings the controller so that it can
   * function as intended. Continuously runs so that input can be received.
   */
  public void start() {
  ...
  public void processCommand(String instruction, Scanner sc) {
    ...
    switch (instruction) {
      ...
      case "load-project":
        ...
        IPixel[][] lpPixels = this.model.doubleArrayRGB(lpHeight, lpWidth);
        ...
        while (sc2.hasNext()) {
          ...
          for (int i = 0; i < lpHeight; i++) {
            for (int j = 0; j < lpWidth; j++) {
              ...
              IPixel pixCurrent = this.model.createRGB(r, g, b, lpMaxValue);
          ...
          layer = this.model.createLayerImp(lpName, list, lpHeight, lpWidth);
