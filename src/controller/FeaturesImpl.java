 ...
 * We use a delegate for code reuse, adhering to the composition strategy.
 */
public class FeaturesImpl implements Features {
  ...
  /**
   * Constructor for FeaturesImpl with model and view implementations provided.
   * The view is remade into a GUI if it is not already one.
   *
   * @param model the project which the user will be editing through this implementation
   */
  public FeaturesImpl(IProject model, IView view) {
    this.model = model;
    this.view = view.makeGui(this);
  ...
  public void saveImg(String imgPath) {
    ...
    // create the delegate with the input
    this.delegate = new SaveImageCmd(new Scanner(imgPath), this.model, this.view);
      ...
      while (scan.hasNextInt()) {
        for (int heightIndex = 0; heightIndex < height; heightIndex++) {
          ...
          for (int widthIndex = 0; widthIndex < width; widthIndex++) {
            ...
            int b = scan.nextInt();
            int a = scan.nextInt();
            IPixel newPixel = this.model.createRGBA(r, g, b, a);
            row.add(newPixel);
          }
          layerPix.add(row);
        }
  ...
  @Override
  public void loiFilter(String fName) {
